...
class LocationProvider with ChangeNotifier {
  ...
  List<String> _restrictedStateList = [];
  List<String> get restrictedStateList => _restrictedStateList;
...
// added by monica for statelist using location repo
  Future<ResponseModel?> getRestrictedDeliveryStateList(BuildContext context) async {
    ResponseModel? responseModel;
    ApiResponse apiResponse = await locationRepo!.getDeliveryRestrictedStateList();
    if (apiResponse.response != null && apiResponse.response!.statusCode == 200) {
      _restrictedStateList = [];
      apiResponse.response!.data.forEach((address) => _restrictedStateList.add(address));
      responseModel = ResponseModel('successful', true);
    }else {
      if (context.mounted){
