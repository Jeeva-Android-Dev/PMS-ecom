...
class ProductProvider extends ChangeNotifier {
 ...
 void changeTypeOfProduct(ProductType type, String? title){
  ...
  // added by monica for customer roles list
  Future<void> getDRolesList()  async {

    ApiResponse  apiResponse = await productRepo!.getDRolesList();
    if(apiResponse.response!= null && apiResponse.response!.statusCode == 200)  {

      // _RoleDashboardModel = RoleDashboardModel.fromJson(apiResponse.response!.data);
      // print('$apiResponse.statusCode} ');
      //_RoleDashboardModel = apiResponse.response!.data['role'];
      _role = apiResponse.response!.data['role'];
      final prefs = await SharedPreferences.getInstance();

      if(apiResponse.response!.data['role']=="customer"){
        await prefs.setString('role','customer');
        if (kDebugMode) {
          print("cus");
        }

      }else if (apiResponse.response!.data['role']=="dealer"){
        await prefs.setString('role','dealer');

        if (kDebugMode) {
          print("dealer");
        }
      }else if (apiResponse.response!.data['role']=="distributor"){
        await prefs.setString('role','distributor');
        if (kDebugMode) {
          print("distributor");
        }
      }
      // apiResponse.response!.data.forEach((RoleDashboardModel) => _RoleDashboardModel!.add(RoleDashboardModel().fromJson(roleDashboardmodel)));
      notifyListeners();
    } else {
      ApiChecker.checkApi(apiResponse);
